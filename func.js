...
var metrics=1;   //выбираем метрику, она влияет на форму получаемых изображений на диаграмме
 ...
 canvas.onclick = function (event) { //Обработчик кликов
  var x = event.clientX - canvas.offsetLeft,//Координаты курсора в момент клика (относительно окна)
      y = event.clientY - canvas.offsetTop;//offsetLeft (расстояние по длине монитора)/offsetTop (расстояние по ширине монитора) 
  					  //offsetLeft/offsetTop используются для кросс-браузерного вычисления координат элемента
